...
module base() {
    ...
    difference() {
        ...
        cylinder(r1=0.4, r2=2.4, h=2.01, $fn=4, center=false);
    }
}
...
//    rotate([180,0,0])
    difference() {
        ...
        for (i = [1:5]) {
          rotate([0, 0, 360/5*i])
            translate([11.3, 0, (-hauteur/2.548)-2.6]) rotate([0,0,0])
...
module decoupe() {
    translate([0, 0, -hauteur/2.548])
        cylinder(h=666, d=666);
}

module plots(largeur, longueur, h) {
    for (i = [1:5]) {
      rotate([0, 0, 360/5*i])
        translate([16.15, 0, (-hauteur/2.548)-(h/2)]) rotate([0,0,0])
//            cylinder(d1=diam-cone, d2=diam, h=h, $fn=4, center=true);
