...
prism_height_ratio=0.68; // 0.3
prism_scale=0.01; // 0.3
nfaces = [];
scale=1;
//////
...
module avec_trou() {
    difference() {
        ...
        translate([radius*0.7, 0, radius]) rotate([0,-45])
            cylinder(h=2*radius, r=2, center=false, $fn=20);
    }
...
module VE_axis(radius, thickness=4, fn=33) {
  h = 2*radius-8;
  r = thickness/2;
  $fn = fn;
  translate([0,0,radius*sqrt(2)/2]) {
    rotate([45])
