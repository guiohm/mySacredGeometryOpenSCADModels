arete = 40;
hauteur = 25.455; // hauteur pyramide
paroi = 1.5;
echelleReductionAjustementBouchon = 0.985;
libEcho=false;


hauteurInterieure = hauteur - 1.1 * paroi;
...
module poly_creux() {
	difference() {
        stellated_hexahedron(arete, hauteur);
        stellated_hexahedron(areteInterieure, hauteurInterieure);
	}
}

module decoupe_bouchon(scale=1) {
    distance = arete/2;
    scale2 = 1.15*pow(scale,8);
	rotate([0,0,45]) translate([0,0, distance+0.01])
		cylinder(999, r=arete*0.8, $fn=4, center=false);
	rotate([0,0,45]) translate([0,0, distance-paroi*scale2+0.01])
		cylinder(paroi*scale2, r=(arete-paroi)*0.71*scale, $fn=4, center=false);
}
...
module supplement_feuillure_bouchon(scale=1) {
    ...
    difference() {   
		cylinder(paroi+1, r=(arete-paroi)*0.71*scale, $fn=4, center=false);
		cylinder(paroi+1, r=(arete-2*paroi)*0.71*scale, $fn=4, center=false);
    }
}

module stellated_hexahedron(side, height) {
    union() {
        cube(side, center=true);
        copy_mirror([1,1,1])
        for (i = [1:3]) {
            rotate(a=360/3*i, v=[1,1,1]) translate([0,0, side/2-0.0001])
            pyramid(side=side, height=height, square=true, centerHorizontal=true, centerVertical=false);
        }
    }
}
...
{
    children();
    rotate(a=360/6, v=[1,1,1]) mirror(vec) children();
}
...
// creates a 3 or 4 sided pyramid.  -1 height= tetrahedron or Johnson's Solid square pyramid
	...
	vert= centerVertical!=true ? [0,0,0]:
		square==false ? [0,0,-mHeight/4] :	 //inscribes a tetra inside a sphere, not 1/2 ht
			[0,0,-mHeight/3];	//for squares, center=false inscribes, so true is just 1/2 ht
